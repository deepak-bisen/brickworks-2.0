...
public interface ProductionLogRepository extends JpaRepository<ProductionLog, String> {
    ...
    // Open ProductionLogRepository.java
    @Query("SELECT SUM(p.quantity) FROM ProductionLog p WHERE p.createdAt >= :startOfDay AND p.createdAt < :endOfDay")
    Long sumProductionByDate(@Param("startOfDay") java.time.LocalDateTime startOfDay,
