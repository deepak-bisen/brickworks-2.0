...
public class JwtRequestFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain chain)
        ...
        // STATELESS MICROSERVICE LOGIC: We DO NOT load UserDetails from a DB!
        // We implicitly trust the signature of the token and extract the role directly.
        if (username != null && SecurityContextHolder.getContext().getAuthentication() == null) {

            if (jwtUtil.validateToken(jwt)) {
                String role = jwtUtil.extractRole(jwt);

                // Spring expects roles to start with ROLE_, which we added during login generation
