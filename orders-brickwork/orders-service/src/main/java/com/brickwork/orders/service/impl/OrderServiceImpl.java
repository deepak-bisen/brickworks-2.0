...
public class OrderServiceImpl implements OrderService {
    ...
    @Autowired
    private ProductClient productClient;  // Feign Client to talk to Product Service
    ...
    public Order createOrderWithStatus(OrderRequestDTO orderRequest, String status) {
        ...
        if (finalCustomerId == null || finalCustomerId.isEmpty() || finalCustomerId.equals("0") && orderRequest.getEmail() != null) {
            try {
                ...
                customerPayload.put("address", orderRequest.getDeliveryAddress());
...
*/
    ...
    public OrderDTO updateOrderStatus(String orderId, String status) {
        ...
        // TODO: In the future, if status is "DISPATCHED", trigger Notification Service here!
    ...
    private OrderDTO mapToDTO(Order order) {
        ...
        dto.setTotalCost(order.getTotalAmount());
    ...
    private void validateOrderRequest(OrderRequestDTO request) {
        ...
        if (request.getDeliveryAddress() == null || request.getDeliveryAddress().length() < 3) {
            throw new IllegalArgumentException("Delivery location/Pincode is too short.");
        }
    }

    // --- Helper Method to convert  responseDTO to Entity---
